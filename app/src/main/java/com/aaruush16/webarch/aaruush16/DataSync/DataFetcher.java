...
public class DataFetcher {
    ...
    public void fetchJSON(final Context context) {
        ...
        JsonArrayRequest teamRequest = new JsonArrayRequest(Request.Method.GET, URLTeam, null,
                new Response.Listener<JSONArray>() {
                    @Override
                    public void onResponse(JSONArray response) {
                        RealmList<PersonTeam> personList = new RealmList<>();
                        int len = response.length();
                        try {
                            for (int i = 0; i < len; i++) {
                                JSONObject jsonObject = response.getJSONObject(i);
                                PersonTeam personTeam = new PersonTeam();
                                personTeam.setImg_id(jsonObject.getString("imgSource"));
                                personTeam.setName(jsonObject.getString("name"));
                                personTeam.setContact(jsonObject.getString("contact"));
                                personTeam.setPosition(jsonObject.getString("pos"));
                                personTeam.setEmail(jsonObject.getString("email"));
                                personTeam.setFb_link(jsonObject.getString("fb"));
                                personList.add(personTeam);
                            }
                            saveDataTeam(personList,context);
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
        ...
        if (isConnected) {
            ...
            AppController.getInstance().addToRequestQueue(teamRequest);
        } else {
    ...
    public void saveDataTeam(final RealmList<PersonTeam> teamList, final Context context) {
        final Realm realm = Realm.getDefaultInstance();
        final Boolean success = true;
        realm.executeTransactionAsync(new Realm.Transaction() {
            @Override
            ...
            public void onSuccess() {
                ...
                Iterator<PersonTeam> eventIterator = results.iterator();
                while (eventIterator.hasNext()) {
                    PersonTeam personTeam = eventIterator.next();
                    Log.w("Realm PersonTeam.class", personTeam.getName());
                    //Toast.makeText(context, "Realm: "+event.getName(), Toast.LENGTH_SHORT).show();
                }
                //Toast.makeText(context, "Workshops Updated!!!", Toast.LENGTH_SHORT).show();
            }
